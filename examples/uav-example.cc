 ...
 * This example uses traces from Remcom. The actual output from Remcom is 
 * adjusted to the same format as the input traces of the QD channel model.
 */
...
main (int argc, char *argv[])
{
  ...
  rxMob->SetPosition (Vector (0, 0, 30));
  txMob = CreateObject<ConstantPositionMobilityModel> ();
  txMob->SetPosition (Vector (0, 0, 10));
  ...
  // Remcom: testing with omni antennas, no beamforming gain.
  txAntenna = CreateObjectWithAttributes<ThreeGppAntennaArrayModel> ("NumColumns", UintegerValue (1),
                                                                     "NumRows", UintegerValue (1),
                                                                     "IsotropicElements", BooleanValue (true));
  txNode->AggregateObject(txAntenna);

  rxAntenna = CreateObjectWithAttributes<ThreeGppAntennaArrayModel> ("NumColumns", UintegerValue (1),
                                                                     "NumRows", UintegerValue (1),
                                                                     "IsotropicElements", BooleanValue (true));
...
ComputeSnr ()
{
  ...
  uint32_t numRbs = 100;
  std::vector<int> activeRbs0 (numRbs);
  for (int i = 0; i < numRbs ; i++)
  {
    activeRbs0[i] = i;
  }
  Ptr<SpectrumValue> txPsd = LteSpectrumValueHelper::CreateTxPowerSpectralDensity (2100, numRbs, txPow, activeRbs0);
  Ptr<SpectrumValue> rxPsd = txPsd->Copy ();
  NS_LOG_DEBUG ("Average tx power " << 10*log10(Sum (*txPsd) * 180e3) << " dB");

  // Create the noise PSD
  Ptr<SpectrumValue> noisePsd = LteSpectrumValueHelper::CreateNoisePowerSpectralDensity (2100, numRbs, noiseFigure);
  ...
  NS_LOG_UNCOND ("Doing beamforming");
  ...
  NS_LOG_UNCOND (Simulator::Now ().GetSeconds () << "\t SNR=" << 10 * log10 (Sum (*rxPsd) / Sum (*noisePsd)) << "dB \t RX Power=" << 10*log10(Sum (*rxPsd) * 180e3)+30 << " dBm");
  f << Simulator::Now ().GetSeconds () << "\t" << (Sum (*rxPsd) / Sum (*noisePsd)) << std::endl;
  f.close ();
